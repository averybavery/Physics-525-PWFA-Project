...
sigma_z = 500e-6   # bunch length [m]

# Super-Gaussian order:
p = 3 # will be raised to 2p
...
print(f"Super-Gaussian order p = {p}")
...
# Super Gaussian drive beam
abs_xi = np.abs(xi)
sgn_xi = np.sign(xi)

n_b = n_b0 * np.exp(-0.5 * (abs_xi / sigma_z)**(2 * p))
...
ax1.set_title(
    "Super-Gaussian drive beam and plasma wakefield\n"
    + rf"$n_0={n_0:.2e}$, "
    + rf"$n_{{b0}}={n_b0:.2e}$, "
